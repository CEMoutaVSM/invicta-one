...
├── scripts/
│   ├── parse_input.py          # Messy input → normalised structure
│   ├── classify.py             # Rule-based pass (regex/heuristics), no LLM
│   └── validate_output.py      # Output must satisfy the contract or fail loudly
├── references/
│   ├── output-contract.md      # Exact schema, section order, enums
│   └── translation-table.md    # Domain jargon → business language
...
└── evals/
    ...
    └── eval-log.md             # Results, appended to SKILL.md on submission
```
