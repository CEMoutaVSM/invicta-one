...
PLAN = [
    ("parse_diff.py", "*.diff", {0}),
    ("load_rules.py", None, {0}),
    ("validate_findings.py", "adversarial-*.md", {1}),
    ("validate_findings.py", "valid-*.md", {0}),
]
...
def run(script, arg=None):
    ...
    if arg:
        cmd.append(str(arg))
    if script in ("load_rules.py", "validate_findings.py"):
        cmd += ["--today", "2026-08-06"]
