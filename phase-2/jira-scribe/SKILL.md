...
- [ ] `validate_output.py <story.md> --parsed <parser JSON>` exits 0.
      **The `--parsed` is not optional.** It is the fabrication guard: without it,
      a number or an actor that appears nowhere in the input passes unchallenged.
...
fresh run, so it cannot describe a case the
suite does not run.
...
output**, and case 5 asserts that a well-formed refusal passes validation rather
